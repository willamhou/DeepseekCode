...
pub fn default_registry_with_context(
    ...
    let mut tools: Vec<Box<dyn Tool>> = vec![
        ...
        Box::new(ExecShellSupervisorStatusTool),
    ...
    if subagent_depth < MAX_SUBAGENT_DEPTH {
        ...
        tools.push(Box::new(RlmLiveStatusTool {
            config: config.clone(),
        }));
        ...
        tools.push(Box::new(RlmLiveRecoverTool {
            config: config.clone(),
        }));
        tools.push(Box::new(RlmLiveStopTool {
            config: config.clone(),
        }));
...
"#,
    ...
    fn default_registry_includes_dispatch_subagent_only_below_max_depth() {
        ...
        assert!(root
            .names_for_policy(&ExecutionPolicy::new(&approval, None))
            .contains(&"rlm_process_recover"));
        assert!(root
            .names_for_policy(&ExecutionPolicy::new(&approval, None))
            .contains(&"rlm_process_stop"));
        assert!(root
            .names_for_policy(&ExecutionPolicy::new(&approval, None))
            .contains(&"rlm_batch"));
        assert!(root
