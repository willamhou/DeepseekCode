...
- 支持本地 HTTP/SSE runtime、ACP stdio adapter、MCP client/server tooling，
  以及 TUI 内的 MCP 管理界面。
- 支持 guided `/setup` onboarding，包括 first-run stepper、provider/model
  picker、TUI masked auth，以及 CLI stdin auth 持久化。
...
- wizard completion/validation 状态、发布渠道安装验证，以及更真实的
  model-backed demo 展示。
