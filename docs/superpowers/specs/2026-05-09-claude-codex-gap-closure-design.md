...
local hooks / config bootstrap / live coverage gate / benchmark asset reproducibility / IDE bootstrap 已收口，最新 benchmark 为 `44/44`，trend gate 已恢复通过，全量测试为 `503 passed, 0 failed`。
...
4. IDE / 编辑器配套仍只有最小入口，MCP/plugin 生态、云端/外部任务面仍缺失
