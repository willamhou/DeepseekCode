...
| `setup wizard`, `/setup wizard` | Open the first-run stepper that sequences provider, model, auth, trust, theme, and language setup |
...
| `skills sync`, `/skills sync` | Download supported TOML, SKILL.md, GitHub, tar.gz, and zip registry entries into the configured skill cache directory |
...
| `skill install <registry-name\|url>`, `/skill install <registry-name\|url>` | Install a DeepSeekCode skill from a direct TOML URL, SKILL.md URL, GitHub repo, tar.gz/zip archive, or registry entry into the configured user skills directory |
